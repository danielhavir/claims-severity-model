...
print(4*'#', 'dataframe loaded'.upper(), 4*'#', end='\n\n')

kf = KFold(n_splits=args.n_splits, random_state=args.seed)

print(4*'#', f'Starting training {args.n_splits} models'.upper(), 4*'#', end='\n\n')

for ii, (train_ids, val_ids) in enumerate(kf.split(df)):
    ...
    print(2*'#', f'Training model #{ii+1}', 2*'#')
    for epoch in range(1, args.epochs+1):
        ...
        if not args.no_checkpoints and epoch_loss < best_loss:
            ...
            torch.save(model, os.path.join('data', 'checkpoints', f'ensemble_{args.model}{ii}.ckpt'))
            print(2*'#', f'Best loss: {best_loss} achieved. Model saved', 2*'#')
        
        scheduler.step()

    with open(os.path.join('data', 'stats', f'ensemble_stats_{args.model}{ii}.json'), 'w') as f:
        json.dump(stats, f)
